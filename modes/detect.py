...
def main():
    ...
    test_transform = Alb.Compose(
        [
            ...
            Alb.LongestMaxSize(max_size=CONSTANTS.IMAGE_SIZE),
    ...
    with torch.no_grad():
        ...
        for i in range(3):
            batch_size, n_anchors, g_S, _, _ = output[i].shape
            anchor = anchors[i]
            boxes_scale_i = convert_cells_to_bboxes(
                                output[i], anchor, s=g_S, is_predictions=True
                            )
    ...
    for i in range(batch_size):
        ...
        nms_boxes = nms(bboxes[i], iou_threshold=0.45, threshold=0.6)
